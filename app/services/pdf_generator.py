...
def generate_pdf_adhoc(content: str, job_title: str = "", company: str = "", cv_text: str = "") -> bytes:
    """Generate a cover letter PDF for the standalone (ad-hoc) flow.

    Identical layout to generate_pdf, except the 'Application for <title> at
    <company>' line is only rendered when BOTH title and company are provided.
    Kept separate so the existing job-based generate_pdf is unaffected.
    """
    info = _extract_contact(cv_text) if cv_text else {"name": "Applicant", "contact": ""}
    ...
    # Application for — only when BOTH title and company are present
    pdf.set_text_color(0, 0, 0)
